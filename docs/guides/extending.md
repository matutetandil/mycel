...
```hcl
type "invoice" {
  number       = string({ validator = "luhn_check" })
  supplier_tax = string({ validator = "cuit" })
  contact      = string({ validator = "company_email" })
}
```
...
Aspects (cross-cutting concerns applied across flows by pattern matching) are a **core concept**, not a runtime extension — they're fully declarative, with no WASM or external code involved. Their documentation now lives in [Core Concepts → Aspects](../core-concepts/aspects.md).
