...
- **A `Subscription` field declared in SDL was ignored.** The schema builder read the Query and Mutation types out of the SDL and skipped Subscription entirely, and the field a flow registered was built from scratch and stored over whatever the declaration would have put there. A field declared to return an object therefore ran as `JSON`, so `subscription { orderPlaced { id } }` — the shape the schema invites — was refused for selecting subfields on a scalar unless the flow repeated the type in a `returns`, and any argument the field declared simply did not exist. `_service { sdl }` published the declaration all along, so a federation gateway composed on a contract the running schema did not implement. A declared field now keeps its type, its arguments and its description, and gains only the two functions that make it a subscription; a flow's `returns` still decides the type, as it does for a query field, and a field nobody declared is `JSON` as before. Found while fixing the above, by checking whether the same class of defect existed in the sibling paths.

  This is a patch by the rule in [Versioning and Support](docs/versioning.md): a declaration that was parsed and never read. One thing to look at when upgrading: a client that asked for such a field *without* a selection — `subscription { orderPlaced }`, which only worked because the field was secretly JSON — now has to select the subfields the schema declares, as it would against any other server.
...
- **`headers` on a `step`, `to` or `enrich`: request headers per request, for the connectors that speak HTTP.** The only headers that reached the wire were the connector's static `headers = {...}` plus auth, and an upstream that selects a tenant, locale or store view by header — the same URL, the same query, a different `Store:` — could only be addressed with one connector per possible value, one step per connector, each guarded with a `when`, and a new store view meant editing every flow. A `headers` attribute written on the step was swept into the connector params, which nothing read, and `mycel validate` said nothing. The attribute now takes CEL expressions or constants, evaluated per request with `input.*` and `step.*` in scope, and is sent over the connector's own headers on the same name by the `http`, `graphql` client and `soap` connectors. A header that evaluates to `null` is not sent. `mycel validate` refuses the attribute on a connector that sends no request headers — a database, a queue — rather than letting it be the next silent no-op. The attribute is accepted as `headers = {...}` on all three blocks and as a `headers {}` block on `step` and `enrich`, including an `enrich` inside a named transform, whose enrichments used to keep only `connector`, `operation` and `params`. A saga's `action`, `compensate`, `on_complete` and `on_failure`, and a state machine transition's `action`, take the same attribute: they make the same call a step does, through the same connectors, and were the one place left where `headers` was refused as an unknown attribute. (#103)

- **`key_from` on a `cache` block: a cache key derived by a CEL expression, for the requests a template of scalars cannot identify.** A cache `key` is a `${...}` template, and since 3.6.2 `mycel validate` refuses one written as CEL — which is right, and which left a list or a map with no expressible key at all. A faceted listing's `items(filter: [{code, value}, ...])` depends on the whole filter set; `${input.filter}` rendered Go's own syntax into the key, with a warning, and the sorted-or-joined-or-hashed string the key needs could not be written anywhere before the lookup ran. `keys_from` and `patterns_from` already take CEL for the same reason on the invalidation side. `key_from` is the read-side twin: evaluated against `input.*` before the lookup, it must yield a non-empty string — a list, a number or an empty string fails the request rather than caching under an empty or literal key, and the error names the flow and the attribute. `key` and `key_from` are mutually exclusive: the parser refuses both, and validate refuses a `key_from` carrying `${...}`, the template form in the wrong attribute. A named cache's prefix goes in front of a derived key as it does a template one. An aspect's `cache {}` block takes `key_from` under the same rules, and `key` there is no longer required when `key_from` is given. (#104)

### Fixed

- **A GraphQL field declared as a list could not be answered by a transform, and a bare step answered it wrongly for zero or one rows.** A `transform` (or `response`) produces an object, and the resolver handed that object to a field declared `[Item!]!`, which failed with `internal error`. The only shape that worked was a flow with no transform whose step happened to return two or more rows: one row was flattened to an object and zero rows to `null`, and both failed the same non-null list field — so a listing that legitimately matched one product was an error. The answer is now fitted to the declared type before it is pruned: an object with exactly one key holding a list is served as that list, a single object is wrapped, and `null` is an empty list. `transform { items = "as_list(step.rows).map(r, {...})" }` answers a list field, whatever the row count. (#105)

- **A GraphQL field declared as a scalar (`String!`, `Int!`) was answered with the stringified map, HTTP 200, no error.** Whatever the flow produced reached graphql-go as an object, and the `String` coercion formatted it with `%v`: the client received `"map[value:hello]"` for a `transform { value = "..." }` and for a one-row step alike. Nothing failed, so the wrong value shipped. A built-in scalar or enum field answered with an object of exactly one key is now served that key's value, and an object with several keys is an error that names the field, its type and the keys, since none of them can be the value. `JSON` and custom scalars are left alone: for those an object is the value, and `Boolean` keeps its own rule. A `Subscription` field is fitted the same way: the published payload went to the subscriber as it was, so a scalar subscription field fed a map had the same `map[value:3]` symptom. (#106)

- **The editor's diagnostics ran their own list of checks, so a configuration `mycel validate` refuses could look clean in the IDE.** `pkg/ide` parsed each file permissively with its own indexer and reported what that could see; `runtime.Checks` — the single list behind `validate` and `start` — was never run. A cache key written as CEL (#100), `headers` aimed at a database (#103), `key` and `key_from` in the same block (#104): each refused at startup, each squiggle-free while being written, and each new runtime check had to be implemented a second time for the editor or it was simply absent there. The editor now parses the indexed buffers with the real parser and runs those checks, placing each finding on the block its message names; the positional checks stay where they are more precise (an undefined reference underlines the reference, not the flow) and the runtime kinds they cover are skipped rather than reported twice. Whole-project checking is suspended while a file has a syntax error, since a half-typed file is not a configuration yet. A test ties the two lists together, so a check added to the runtime is either surfaced here or explicitly listed as already covered. (#108)

  Two things underneath it are worth naming. `parser.ParseSources` parses a configuration from file contents held in memory — the editor's unsaved buffers — with the same passes `Parse` runs over a directory, so what is checked is what the user is typing rather than what they last saved. And a parse error now names the block at fault (`flow "orders" parse error: ...`), which is what lets a finding land on that block; the text of those errors changed accordingly.

- **A response served from the cache rounded integers larger than 2^53.** The cache stores JSON and decoded it into an `interface{}` with `encoding/json`, which reads every number as a `float64`; a `float64` holds 53 bits of integer, so an id, a 64-bit hash or a large amount came back from the cache with its low digits rounded away, while the first, uncached answer had them right — `4244637855813302974` on the first request, `4244637855813303000` on the second, same entry. Seen live while verifying `key_from`. The decode now keeps the digits: an integral number that fits an `int64` is an `int64`, which is also what the flow produced, and anything else is a `float64` as before. The wire format has not moved, so entries already stored, and entries written by a service that is not Mycel, read the same. (#109)
...
- **The REST connector never decoded a `DELETE` request body, so a `DELETE` flow with a JSON body received an empty input.** The body was decoded for `POST`, `PUT`, `PATCH` and `QUERY` and for nothing else; a flow declared as `DELETE /keys` with `{"ids": [...]}` got no fields — not an error, an empty input, which falls through to whatever the "no arguments" branch of the flow does. In the service that found it that branch was "clear everything", so a precise `DELETE` would have flushed the whole cache on every call and the only symptom would have been a cache that never held anything. RFC 9110 does not forbid a body on `DELETE` and "delete these ids" is how a selective endpoint is normally shaped. `DELETE` now decodes like the others: a body-less `DELETE` is unchanged, and a body that does not parse is a `400` rather than an empty input, same as every other method. (#98)

- **`??` inside a map literal swallowed the key, producing CEL that does not compile.** `{'open': input.open ?? ''}` was rewritten to `{coalesce('open': input.open, '')}` — the rewrite assumed the left operand began where the enclosing argument did, which holds after `(`, `,` and `[` and not after the `:` of a map entry. `mycel validate` passed, since it does not compile CEL, and the request failed with `Syntax error: missing ')' at ':'`, which reads as map literals being unsupported rather than as a rewrite bug. Building a nested object with a default per field is most of what a response transform does over flat SQL rows. The rewrite now folds each operand on its own, stopping at a map key and at a ternary branch, which also gives `??` the precedence it has in JavaScript and C# — `c ? a ?? 1 : b` is `c ? (a ?? 1) : b` — where the two used to be documented as not resolvable without parentheses. (#99)

- **The caching guide wrote cache keys as CEL concatenation, and a key written that way gives every record the same entry.** A cache `key` is a template — `${...}` is substituted and the rest is the key verbatim — but `docs/guides/caching.md` showed `key = "'product:' + input.id"` in its table, its reference section and four examples, and five more pages copied it. The form is the one `lock`, `dedupe` and `coordinate` take, so it reads as right, and nothing rejects it: not validate, not startup, not the request. The key is used with its quotes and `+` intact, so every product shares one cache entry, the first request fills it and every later request for any id gets that product back for the life of the TTL. The symptom is users seeing the wrong record, which looks nothing like a cache key. All twelve occurrences now show the template form and the guide says, in its own admonition, which keys are CEL and which are not. `mycel validate` refuses a cache key — on a flow or an aspect — whose text outside `${...}` carries quotes, a `+` or an `input.` reference, and its message shows the template to write instead. (#100)
