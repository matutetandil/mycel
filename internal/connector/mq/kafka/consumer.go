...
// startReaderOnly creates the Kafka reader without starting consume loops.
// Used in manual consume mode where messages are fetched via ConsumeOne().
func (c *Connector) startReaderOnly(ctx context.Context) error {
	consumerCfg := c.config.Consumer
	if consumerCfg == nil {
		return fmt.Errorf("consumer configuration is required")
	}
	...
	readerConfig := kafka.ReaderConfig{
		...
		MaxWait:        consumerCfg.MaxWaitTime,
		StartOffset:    startOffset,
		CommitInterval: 0, // Manual commit in ConsumeOne
	}
	...
	c.logger.Info("manual consume mode: reader ready, waiting for debug.consume",
		"group_id", consumerCfg.GroupID,
		"topics", consumerCfg.Topics,
	)
