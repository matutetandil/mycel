...
|---|---|---|---|---|
| 11 | `dsn` on a database connector | 14 blocks across `docs/guides/use-cases.md` write `dsn = env("DATABASE_URL")`, and `dsn = "./data.db"` for SQLite | **no such attribute anywhere.** PostgreSQL and MySQL take discrete `host`/`port`/`database`/`user`/`password`; only MongoDB has a single-string `uri`. Every one of those blocks fails to parse | **probably build it.** Somebody wrote `dsn` fourteen times because it is the name everyone uses, and `DATABASE_URL` is the de facto standard on every PaaS and in Docker Compose. Decomposing it by hand into five attributes is real friction. Parsing a connection string into the fields the factories already read is contained work |
