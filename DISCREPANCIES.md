...
|---|---|---|---|
| U7 | **Account linking, after the fact** | Linking happens during a sign-in, by matching a verified email, and that works. `AccountLinkingService` also has confirm, unlink and list, and `sso { linking { match_by, on_match, ... } }` is documented — the parser rejects the block, `AccountLinkingConfig` is always built as nil, and the auth guide listed `/auth/link/:provider`, `/auth/unlink/:provider` and `/auth/linked-accounts`, none of which exist | Parse the `linking` block into the config the service already takes, and mount the three endpoints. Removed from the guide for now rather than left as a promise |
| U8 | **A cache connector as a flow destination** — next release | The cache connector implements Get, Set, Delete, DeletePattern, Exists and TTL, and implements none of `Caller`, `Reader` or `Writer`, so a flow cannot write to it. Redis is reachable today only through a flow's `cache {}` block, which is cache-aside over something else, and from aspects — never as the destination itself. So a service whose job *is* the cache cannot be written: `from tcp → to redis(get/set/delete)` has nowhere to land | Implement `Call` on the cache connector, mapping the operation to the methods that already exist. It is the same gap that was fixed for HTTP in 2.11.0, where `isCallOperation → Caller.Call` was dispatched to and only exec, file, s3, grpc and graphql implemented it. Two things stay open beyond it and are design rather than wiring: a memory tier in front of a Redis one, with the cross-invalidation that a pub/sub currently does by hand, and a namespace chosen per request rather than fixed in the configuration — both wanted by the Mercury cache service this comes from |
| U4 | **The root auth block takes no part in a request** (widened; was "auth.Middleware is unused") | Measured against a running service: `auth` is not a declared CEL variable at all, so `auth.user_id` and `auth.claims.*` — documented in the auth guide and used by `examples/dynamic-api-key` — cannot be written anywhere; the expression fails to compile. A request with an invalid credential and one with none both answered 500 from that compile error rather than 401, so the `provider` block validates nothing that arrives. The REST connector authenticates with its own implementation, builds an `AuthContext`, and `GetAuthContext` is called by nobody. `auth.Middleware`, with its path rules and roles, has no HCL tags and no block in the parser, so unlike the others nothing in the language points at it | **Three pieces, and the first two are the documented behaviour.** Declare `auth` in the CEL environment; carry the identity the connector already builds into the activation; and let a connector enforce through the manager when a root auth block exists, which is what makes providers, sessions, revocation and MFA state count for an incoming request. `auth.Middleware` is then either the shape that enforcement takes or is removed — it is the only piece with no configuration behind it |
