...
| the admin server's workflow endpoints | `GET /workflows/{id}`, `POST /workflows/{id}/signal/{event}`, `POST /workflows/{id}/cancel` are mounted on the admin port with no authentication whenever a workflow engine is configured | **a design decision, recorded rather than changed.** The other admin endpoints are read-only (health, metrics) or gated (the Studio debug protocol); these three mutate business state — anything that can reach :9090 can cancel a running workflow or wake a paused one with data it chose. In Kubernetes that port is normally not published, but it is commonly scraped, so "internal" means "anything in the cluster". Fixing it means either gating them behind the root `auth {}` or moving them off the admin port, and either changes what someone's tooling can already reach: the user's call. The malformed-body defect found alongside it (a signal whose body is not JSON woke the workflow with no data and answered 200) is fixed on this branch | open |
